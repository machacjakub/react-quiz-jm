...
const Item = ({item, onClick}: {item: IItem, onClick: () => void}) => {
    return (
        <button key={item.name} style={{ width: 200, margin: 10, padding: 10, border: '1px solid #bbb', backgroundColor: item.selected ? 'hsl(200, 100%, 80%)' : '#FAFAFA' , cursor: "pointer", borderRadius: 10, lineHeight: '10px' }} onClick={onClick}>
...
const App = ({ boxers }: { boxers: IBoxer[] }) => {
    ...
    return (
        <div style={{display: 'flex', flexWrap: 'wrap' }}>
            ...
            <div>{items.map((boxer) => <Item item={boxer} onClick={() => selectItem(boxer)}/>)}</div>
        </div>
    )
}
